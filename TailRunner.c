...
void TailRunner_set_tailMotor(TailRunner* this)
{
  ...
  switch(this->phase){
    ...
    case 2:
      ...
      if(this->runtime >= 2000){//少したったら
        ecrobot_sound_tone(349, 100, 100);
        this->phase = 3;
        this->runtime = 0;
        LineTracer_changePID(this->lineTracer, 0.75, 0.1, 0.25, get_TARGET_tail(this->lineTracer));
      }
      this->runtime+=4;
      break;
    case 3: 
      TailRunner_changeTailAngle(this, 65);
      LineTracer_trace_nonbalance(this->lineTracer, 0, 0);
      if(this->runtime > 1000){//少し進んだら
        ecrobot_sound_tone(349, 100, 100);
        this->phase = 4;
        this->runtime = 0;
        Maimai_setArrayValue(this->maimai, get_TARGET_tail(this->lineTracer));
      }
      this->runtime+=4;
      break;
    default: break;
